    ...
    public partial class frmBankayaCekCikisi : DevExpress.XtraEditors.XtraForm
    {
        ...
        void YeniKaydet()
        {
            try
            {
                ...
                DB.SubmitChanges();
                Mesajlar.YeniKayıt(txtCekNo.Text + " nolu çekin banka hareket kaydı işlenmiştir.");
                Cek.VERILENBANKA_BELGENO = txtBelgeNo.Text;
                Cek.VERILENBANKA_TARIHI = DateTime.Parse(txtTarih.Text);
                Cek.VERILANBANKAID = BankaID;
                Cek.DURUMU = "Bankada";
                DB.SubmitChanges();
                Mesajlar.YeniKayıt(txtCekNo.Text + " nolu çekin durum güncellemesi yapılmıştır.");
                Temizle();
            }
            catch (Exception EX)
            {
